#!/usr/bin/env python3
"""Twin.me 2.0 — 라이트 모드 E2E 유저 여정 검수서 PDF 빌더"""
import json, os, sys
from pathlib import Path
import img2pdf

ROOT     = Path(__file__).parent.parent
MANIFEST = ROOT / "light_manifest.json"
OUTPUT   = ROOT / "Twin.me_2.0_E2E_유저여정_전체구조_검수서_라이트모드.pdf"
...
def main():
    ...
    print(f"\n📄 PDF 통합 시작 — {len(manifest)}컷 (라이트 모드)\n")
    pages = []
    for entry in manifest:
        ...
        if cap_fp and os.path.exists(cap_fp):
            ...
            print(f"  ❌ [{str(i).zfill(2)}] 파일 없음")

    if not pages: print("❌ 결합할 이미지 없음"); sys.exit(1)
