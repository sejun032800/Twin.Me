/**
 * Twin.me 2.0 — E2E 유저 여정 전체 구조 캡처
 *
 * 실제 사용자가 앱을 다운로드하고 터치하며 이동하는 순서 그대로:
 *   1. 온보딩 진입
 *   2. 채팅 목록 (분석가 트윈이 배너)
 *   3. 트윈이 챗룸 (리포트 버블)
 ...
 *  15. 계정 삭제 2단계
 *
 * 출력: Twin.me_2.0_E2E_유저여정_전체구조_검수서.pdf
 */
...
const SCALE  = 2;
const CAP_H  = 84;  // caption bar height (logical px)

// ── 섹션별 테마 색상 ─────────────────────────────────────────────────────────
const SECTION = {
  ONBOARD : { color: '#B482EE', label: '① 온보딩 진입' },
  CHAT    : { color: '#CE93D8', label: '② 채팅 & 리포트' },
  PAYWALL : { color: '#F48FB1', label: '③ 페이월 & 결제' },
  MEMORY  : { color: '#81D4FA', label: '④ 추억 & 무드 피드' },
  MAP     : { color: '#A5D6A7', label: '⑤ 데이트 지도' },
  SETTING : { color: '#FFCC80', label: '⑥ 설정 & 계정 관리' },
  DELETE  : { color: '#EF9A9A', label: '⑦ 2단계 회원 탈퇴' },
};
...
async function snap(page, caption, section) {
  idx++;
  const raw = path.join(ROOT, `e2e_raw_${String(idx).padStart(2, '0')}.png`);
...
// ── Playwright HTML 캡션 렌더러 ──────────────────────────────────────────────
async function renderCaptions(capPage, outDir) {
  console.log(`\n${'─'.repeat(60)}`);
  console.log('🎨 캡션 합성 (Playwright HTML 렌더러) — Noto Sans KR\n');
  ...
  for (const entry of manifest) {
    ...
    const { color, label } = SECTION[section] ?? { color: '#B482EE', label: section };
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .num {
    background:${color}; color:#0A0D1A;
    font-weight:700; font-size:15px;
    padding:5px 11px; border-radius:7px;
    flex-shrink:0; white-space:nowrap;
  }
  .cap {
    color:#F1F5F9; font-size:13.5px; font-weight:400;
    line-height:1.45; flex:1;
  }
  .sec {
    color:${color}; font-size:10.5px; font-weight:300;
...
<body>
  ...
  <div class="bar">
    ...
    <span class="sec">${label.replace('①','').replace('②','').replace('③','').replace('④','').replace('⑤','').replace('⑥','').replace('⑦','').trim()}</span>
  </div>
</body>
</html>`;
    ...
    const out = path.join(outDir, `e2e_captioned_${String(i).padStart(2,'0')}.png`);
...
// ════════════════════════════════════════════════════════════════════════════
(async () => {
  console.log('\n🚀 Twin.me 2.0 — E2E 유저 여정 전체 구조 캡처 시작');
  console.log('   iPhone 14 뷰포트 393×852 @2x  |  Noto Sans KR 한글 주입\n');
  console.log('═'.repeat(60));

  // clean previous raw files
  for (let n = 1; n <= 20; n++) {
    const f = path.join(ROOT, `e2e_raw_${String(n).padStart(2,'0')}.png`);
    if (existsSync(f)) rmSync(f);
  }

  const OUT_DIR = path.join(ROOT, 'e2e_captioned');
  mkdirSync(OUT_DIR, { recursive: true });

  const browser = await chromium.launch({ headless: true });

  // main capture context (iPhone 14)
  ...
  // ──────────────────────────────────────────────────────────────────────────
  const lockTargets = [
    '비밀 로그', '프리미엄 전용', '잠금 해제', '전체 보기', 'PREMIUM', 'Premium',
  ];
  ...
  if (!nudgeOpened) {
    // 블러 영역 좌표 직접 클릭 (리포트 모달 중하단 ~ y=550)
    await page.mouse.click(196, 550);
    await wait(1400);
  }
  await snap(page, '5. 결제 넛지 팝업 — Coffee Break 구독 업그레이드 유도 CTA', 'PAYWALL');

  // ──────────────────────────────────────────────────────────────────────────
  // 6. 가상 결제 성공 → 블러 해제 스프링 모션
  // ──────────────────────────────────────────────────────────────────────────
  const payBtns = ['Coffee Break', '구독', '업그레이드', '결제', 'Subscribe', '₩'];
  for (const t of payBtns) {
    const el = page.getByText(t, { exact: false }).first();
  ...
  // 9. 무드 필터 토글 활성
  // ──────────────────────────────────────────────────────────────────────────
  const filterTargets = ['내 현재 OOTD', 'OOTD', '무드 필터', '필터'];
  ...
  if (await hamBtn.count() > 0) {
    ...
    // coordinate fallback: top-right of map area
  ...
  await snap(page, '13. 소셜 계정 연동 화면 — 4개 프로바이더 버튼 초기 상태', 'SETTING');

  // connect all 4 social providers in sequence
  ...
  await snap(page, '13. 소셜 계정 연동 완료 — Google·Kakao·Naver·Apple 4개 배지 활성', 'SETTING');
  ...
  // scroll to bottom to expose delete button
  await page.evaluate(() => {
    const scrollables = Array.from(document.querySelectorAll('*'))
      .filter(el => el.scrollHeight > el.clientHeight + 100);
    scrollables.forEach(el => { el.scrollTop = 99999; });
  });
  await wait(700);
  ...
  await snap(page, '14. 설정 최하단 — 로그아웃 · 계정 삭제 레드 버튼 노출 상태', 'DELETE');

  if (await delBtn.count() > 0) { await delBtn.click().catch(() => {}); await wait(1600); }
  await snap(page, '15. 계정 삭제 1단계 — "정말로 삭제하시겠어요?" 경고 알림', 'DELETE');

  const step2Btn = page.getByText('이어하기', { exact: false }).first();
  if (await step2Btn.count() > 0) { await step2Btn.click().catch(() => {}); await wait(1600); }
  await snap(page, '16. 계정 삭제 2단계 — 데이터 파기 요약 & 최종 동의 모달', 'DELETE');
  ...
  // save manifest
  writeFileSync(
    path.join(ROOT, 'e2e_manifest.json'),
    JSON.stringify(manifest, null, 2),
  );

  console.log('\n📋 e2e_manifest.json 저장 완료');
  console.log(`📁 캡션 합성 디렉토리: ${OUT_DIR}`);
  console.log(`📸 총 캡처 컷 수: ${idx}컷\n`);
})();
